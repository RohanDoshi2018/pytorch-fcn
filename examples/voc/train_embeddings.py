#!/usr/bin/env python
# FCN, except 21-class softmax ouput is replaced with pixel-embeddings.
import argparse
import datetime
import os
...
def main():
    ...
    train_loader = torch.utils.data.DataLoader(
        torchfcn.datasets.SBDClassSeg(root, split='train', transform=True, pixel_embeddings=embed_dim),
	batch_size=1, shuffle=True, **kwargs)
    val_loader = torch.utils.data.DataLoader(
        torchfcn.datasets.VOC2011ClassSeg(
            root, split='seg11valid', transform=True, pixel_embeddings=embed_dim),
        batch_size=1, shuffle=False, **kwargs)

    # 2. model
    if embed_dim:
        model = torchfcn.models.FCN32s(n_class=embed_dim)    
    else:
